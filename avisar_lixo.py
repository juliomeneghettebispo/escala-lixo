...
def aba_do_mes(data):
    """Retorna o nome da aba (ex.: 'Set26') correspondente ao mes/ano de uma data."""
...
def carregar_escala(datas_necessarias):
    """
    Carrega e mescla as linhas de TODAS as abas (meses) necessarias para
    cobrir as datas informadas. Isso e essencial para a checagem semanal
    (segunda a sabado), que pode cruzar a virada do mes — ex.: segunda
    31/08 com sabado 05/09 precisa das abas 'Ago26' E 'Set26'.
    """
    # aba -> ano a usar ao interpretar dias sem ano explicito na planilha
    abas_anos = {}
    for d in datas_necessarias:
        abas_anos[aba_do_mes(d)] = d.year

    escala = []
    for aba, ano in abas_anos.items():
        gid = GIDS_MESES.get(aba)
        if not gid or gid == "PREENCHER":
            print(f"AVISO: GID da aba '{aba}' nao preenchido — dias dessa aba serao ignorados.")
            continue
        for row in csv.reader(io.StringIO(baixar_csv(gid))):
            if not row or not row[0].strip() or not row[0][0].isdigit(): continue
            d_str, nome = row[0].strip(), row[2].strip() if len(row)>2 else ""
            if not nome: continue
            try:
                data = datetime.strptime(f"{d_str}/{ano}" if d_str.count("/")==1 else d_str, "%d/%m/%Y").date()
...
def main():
    ...
    # Datas que precisam estar cobertas pela escala carregada:
    # - se for segunda, a semana toda (segunda a sabado), que pode
    #   atravessar a virada do mes;
    # - nos demais dias, so o dia de hoje.
    if is_seg:
        datas_necessarias = [hoje + timedelta(days=i) for i in range(6)]  # seg..sab
    else:
        datas_necessarias = [hoje]

    abas_necessarias = sorted({aba_do_mes(d) for d in datas_necessarias})

    print(f"{'='*50}")
    print(f"Data: {hoje.strftime('%d/%m/%Y')} | Aba(s) necessaria(s): {', '.join(abas_necessarias)}")
    print(f"{'='*50}")
    ...
    escala = carregar_escala(datas_necessarias)
